...
/*
这段代码实现的功能是在给定的函数内部的每个基本块中插入 phi 指令。

首先，通过遍历所有基本块并检查每个基本块内的所有指令，找到活跃在多个基本块的全局名字集合，以及它们所属的基本块。

然后，通过支配树获取每个基本块的支配边界信息，并在支配边界上插入 phi 指令。在插入 phi 指令时，需要避免重复插入。

最后，在所有基本块内插入完 phi 指令后，通过遍历所有基本块并检查每个基本块内的所有指令，更新所有使用了插入的 phi 指令的指令的操作数。
*/
void Mem2Reg::generate_phi() {
    ...
    for (auto &bb1 : func_->get_basic_blocks()) {   // 遍历所有基本块
        auto bb = &bb1;
        std::set<Value *> var_is_killed;
        for (auto &instr1 : bb->get_instructions()) {   // 检查基本块内的所有指令
            auto instr = &instr1;
            if (instr->is_store()) {
                // store i32 a, i32 *b
                ...
                if (!IS_GLOBAL_VARIABLE(l_val) && !IS_GEP_INSTR(l_val)) {   // 找到活跃在多个基本块的全局名字集合，以及它们所属的基本块
...
/*
这段代码实现了从函数中删除所有的 alloca 指令。

首先，它遍历函数中的所有基本块，并且将其中所有的 alloca 指令保存在一个名为 wait_delete 的列表中。

然后它检查该指令的类型是否是整数类型或浮点类型。如果是，就将这些指令从基本块中删除。

这样，函数就不会再使用分配内存的指令，并且优化后的代码只会包含基本块内部变量的操作，而不会再使用内存分配指令。
*/
